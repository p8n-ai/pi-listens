...
export class SarvamVoiceProvider implements VoiceProvider {
	...
	describe(): VoiceProviderInfo {
		...
		const key = sarvamApiKey(config);
		return {
			id: this.id,
			name: this.name,
			authLabel: "Sarvam API key",
			authConfigured: Boolean(key),
			authStatus: key ? "set" : "missing",
			sttSummary: `${config.sttModel} (${config.translateInputToEnglish ? "translate→English" : config.sttMode}, ${config.sttLanguageCode})`,
			ttsSummary: `${config.ttsModel} (${config.ttsLanguageCode}, speaker ${config.ttsSpeaker})`,
			configTemplate: {
				provider: "sarvam",
			...
			setupInstructions: [
				"Open the file and replace sarvamApiKey with your Sarvam AI API key.",
				"Get a key at: https://dashboard.sarvam.ai",
			],
		};
	}

	async transcribeMicrophone(audio: AudioRuntime, signal?: AbortSignal, options: TranscriptionOptions = {}): Promise<TranscriptionResult> {
		const config = this.getConfig();
	...
	async transcribeFile(path: string, signal?: AbortSignal, options: TranscriptionOptions = {}): Promise<TranscriptionResult> {
		const config = this.getConfig();
	...
	private async withStreamingSocket(
		...
		const socket = connectStreamingSocket(config, asSarvamSttMode(mode ?? (config.translateInputToEnglish ? "translate" : config.sttMode)), inputAudioCodec);
	...
	private getClient(config: PiListensConfig): SarvamAIClient {
		const key = sarvamApiKey(config);
		if (!key) {
			throw new Error("Sarvam API key is not configured. Set sarvamApiKey, SARVAM_API_KEY, or run /voice-init.");
		}
...
function sarvamApiKey(config: PiListensConfig): string | undefined {
	return config.sarvamApiKey ?? config.apiKey;
}

function asSarvamSttMode(mode: string): SttMode {
	const allowed = new Set<SttMode>(["transcribe", "translate", "verbatim", "translit", "codemix"]);
	return allowed.has(mode as SttMode) ? (mode as SttMode) : "transcribe";
}
